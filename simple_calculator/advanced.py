...
def parse_args(args=None):
    ...
    # 複利計算の定義
    compound_parser = subparsers.add_parser("compound", help="Compound interest calculation")
    compound_parser.add_argument("principal", type=float, help="Principal amount")
    compound_parser.add_argument("rate", type=float, help="Annual interest rate (as decimal)")
    compound_parser.add_argument("time", type=float, help="Time in years")
    compound_parser.add_argument("--compounds", type=int, default=1, help="Compounds per year")
...
def main(args=None):
    ...
    try:
        ...
        #     pass
            ...
            print(f"Error: Unknown advanced operation '{parsed_args.operation}'", file=sys.stderr)
            sys.exit(1)
